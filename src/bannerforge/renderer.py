...
from .fonts import DEFAULT_TEXT_FONTS, DEFAULT_TITLE_FONTS, safe_font
...
class StyleProtocol(Protocol):
    secondary: tuple[int, int, int]  # or Tuple[int, int, int, int] if RGBA


# local loader
def _load_image_sync(
    source: str | None, expected_size: tuple[int, int] | None = None
) -> Image.Image | None:
    if not source:
        return None
    try:
...
def shadow(
    ...
    canvas = Image.new(
        "RGBA",
        (image.size[0] + abs(offset[0]) * 2, image.size[1] + abs(offset[1]) * 2),
        (0, 0, 0, 0),
    )
...
def _draw_urls(
    ...
    left: int,
    y: int,
    _max_w: int,  # Unused but kept for API compatibility
    sty: StyleProtocol,
    site_url: str | None,
...
def generate_banner(  # noqa: PLR0915
    speaker_name: str,
    talk_title: str,
    date: str,
    venue: str,
    ...
    W, H = size
    sty = (
        PALETTES.get(palette, PALETTES["emerald"])
        if not palette_from
        else palette_from_image("custom", palette_from)
    )
    ...
    if style == "gradient":
        ...
        ImageDraw.Draw(pill).rounded_rectangle(
            (0, 0, pill.width, pill.height), radius=48, fill=(*sty.primary, 255)
        )
        canvas.alpha_composite(pill, (MARGIN, MARGIN))
    elif style == "glass":
        frosted = Image.new(
            "RGBA", (W - 2 * MARGIN, H - 2 * MARGIN), (255, 255, 255, 48)
        )
        ImageDraw.Draw(frosted).rounded_rectangle(
            (0, 0, frosted.width, frosted.height), radius=32, fill=(255, 255, 255, 48)
        )
    ...
    if avatar is not None:
        if style in {"split", "pill-left"}:
            ...
            aimg = Image.alpha_composite(
                aimg, Image.new("RGBA", (aw, ah), (0, 0, 0, 30))
            )
    ...
    title_font, title_txt = auto_shrink_text(
        draw, talk_title, DEFAULT_TITLE_FONTS, title_max_w, max_font=96, min_font=40
    )
    title_y = int(H * 0.18)
    draw.multiline_text(
    ...
    draw.text(
        (content_left, title_y + title_font.size * (title_txt.count("\n") + 1) + 18),
        speaker_name,
        font=speaker_font,
        fill=sty.secondary,
    )
    ...
    meta_y = (
        title_y
        + title_font.size * (title_txt.count("\n") + 1)
        + 18
        + speaker_font.size
        + 14
    )
    ...
    if hashtag:
        ...
        draw.rounded_rectangle(
            (content_left, row_y, content_left + tw, row_y + th),
            radius=th // 2,
            fill=(sty.accent[0], sty.accent[1], sty.accent[2], 220),
        )
        draw.text(
            (content_left + 14, row_y + (th - tag_font.size) // 2 - 2),
            hashtag,
            font=tag_font,
            fill=(255, 255, 255),
        )
    ...
    row_y = _draw_urls(
        draw, content_left, row_y, title_max_w, sty, site_url, telegram_url
    )
